...
WORKDIR /usr/share
RUN wget http://download.elastic.co/elasticsearch/release/org/elasticsearch/distribution/tar/elasticsearch/2.4.1/elasticsearch-2.4.1.tar.gz -O - | tar xvfz - \
    && mv elasticsearch-2.4.1 elasticsearch && cd elasticsearch && rm -rf config bin
